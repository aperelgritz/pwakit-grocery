...
// Alexis custom - check if shopper context exists
export const isShopperContextSet = async ({api, usid, token}) => {
    try {
        const res = await api.shopperContexts.getShopperContext({
            parameters: {usid},
            headers: {Authorization: `Bearer ${token}`}
        })
        console.log('Shopper context set:', res)
        return res
    } catch (e) {
        console.log('Shopper context not set')
        return false
    }
}

// Alexis custom - get store from shopper context
export const getStoreInShopperContext = async (usid, token, api, shopperContextRes) => {
    const storeId = shopperContext.assignmentQualifiers.storeId
